    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="commerceml-sync-data-setting-variants">
                <select id="elm_sync_data_commerceml_cml_catalog_convertor_brand_source" name="sync_data_settings[{$sync_provider_id}][catalog_convertor.brand_source]">
                    {foreach $settings_schema["catalog_convertor.brand_source"].variants as $variant}
                        <option value="{$variant}" {if $import_settings["catalog_convertor.brand_source"] == $variant}selected="selected"{/if}>{__("commerceml.cml_catalog_convertor_brand_source.{$variant|lower}")}</option>
                    {/foreach}
                </select>
                <div class="shift-top">
                    <input type="text" name="sync_data_settings[{$sync_provider_id}][catalog_convertor.brand_property_source]" id="elm_sync_data_commerceml_cml_catalog_convertor_brand_property_source" size="32" value="{$import_settings["catalog_convertor.brand_property_source"]}" />
                </div>
            </td>
            <td>
                <input type="hidden" name="sync_data_settings[{$sync_provider_id}][catalog_importer.allow_update_product_brand]" value="Y">
                <input type="checkbox" name="sync_data_settings[{$sync_provider_id}][catalog_importer.allow_update_product_brand]" value="N" {if !$import_settings["catalog_importer.allow_update_product_brand"]}checked{/if}>
            </td>
        </tr>
