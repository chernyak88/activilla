...
class ProductImporter
{
    ...
    private function fillProductFeatures(ProductDto $product, ImportStorage $import_storage, array $product_data, array $current_product_data)
    {
        ...
        if (!$product->is_new && !$import_storage->getSetting('catalog_importer.allow_update_product_brand', false)) {
            $product_features = $this->product_storage->findProductFeatures(array_keys($product_feature_values));

            foreach ($product_features as $feature_id => $product_feature) {
                if ($product_feature['feature_type'] !== ProductFeatures::EXTENDED) {
                    continue;
                }
                unset($product_feature_values[$feature_id]);
            }
        }
