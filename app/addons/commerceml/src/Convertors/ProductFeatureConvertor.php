...
class ProductFeatureConvertor
{
    ...
    public function convert(SimpleXmlElement $element, ImportStorage $import_storage)
    {
        ...
        if (
            $import_storage->getSetting('catalog_convertor.brand_source', 'none') === 'property'
            && self::isBrandFeature($element->getAsString('name', ''), $import_storage)
        ) {
    ...
    {
        return mb_strtolower(trim($import_storage->getSetting('catalog_convertor.brand_property_source', 'none'))) === mb_strtolower(trim($feature_name));
    }
}
