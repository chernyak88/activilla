...
class ExportOrdersCommandHandler
{
     ...
     * @param array<string, int|string|array>                 $last_sync_info Last synchronization info
     */
    public function __construct(
        ...
        $this->last_sync_info = $last_sync_info;
    }
     ...
     */
    public function getParams(ExportOrdersCommand $command)
    {
        $params = [
        ...
        if ($this->settings['orders_exporter.strategy'] === OrderFormator::STRATEGY_NEW) {
            $params['period'] = DateTimeHelper::PERIOD_CUSTOM;
            $params['time_from'] = (int) $this->last_sync_info['last_sync_timestamp'] - SECONDS_IN_HOUR;
        }
