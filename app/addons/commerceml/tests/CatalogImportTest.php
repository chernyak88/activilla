...
class CatalogImportTest extends StorageBasedTestCase
{
    ...
    public function testUpdateProductBrand()
    {
        ...
        foreach ($product_storage->product_features as $feature_id => $product_feature) {
            if (empty($product_feature[$product_storage->default_language_code]['feature_type']) || $product_feature[$product_storage->default_language_code]['feature_type'] !== ProductFeatures::EXTENDED) {
                continue;
            }
        ...
        $import_storage = $import_storage->newInstance([
            'catalog_convertor.brand_source' => 'manufacturer',
            'catalog_importer.allow_update_product_brand' => true
        ]);
        $this->import($handler, $import_storage, $product_storage);
        ...
        $import_storage = $import_storage->newInstance([
            'catalog_convertor.brand_source' => 'manufacturer',
            'catalog_importer.allow_update_product_brand' => false
        ]);
        $this->import($handler, $import_storage, $product_storage);

        $this->assertEquals(500, $product_storage->products[1]['ru']['product_features'][$brand_feature_id]);
    }
    ...
    public function testImportFeature()
    {
        ...
        $this->assertCount(1, $product_storage->products[1]['ru']['product_features']);
    }
